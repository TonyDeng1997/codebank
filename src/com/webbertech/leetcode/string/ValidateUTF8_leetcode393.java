...
/*
 * leetcode 393 Validate UTF8 character
 * 
...
public class ValidateUTF8_leetcode393 {
	 
	//solution1: using bit mask
	public static boolean validUtf8I(int[] data) {
		...
		for(int i=0;i<data.length;i++) {
			...
			if((data[i] & 128) == 0) { // 0xxxxxxx, 1 byte, 128(10000000)
				numberOfBytes = 1;
			} else if((data[i] & 224) == 192) { // 110xxxxx, 2 bytes, 224(11100000), 192(11000000), mask, value pair
				numberOfBytes = 2;
			...
			//because the next n bytes (numberOfBytes) are associated with the previous number, so i jumps to the next one
			//for example, 11000101 10000010 00000001, it checks 100.., then it jumps to the last one
			// take this example, whe i = 0, numberOfBytes is 2, then i on the rhs is 1, then i++, i == 2.
			i=i+numberOfBytes-1;
		}
		return isValid;
	}
    ...
    // Hard code "masks" array to find the index of the first appearance of 0 in the lower 8 bits of
	// each integer.
    private static int[] masks = {128, 64, 32, 16, 8};
    public static boolean validUtf8II(int[] data) {
        int len = data.length;
        for (int i = 0; i < len; i ++) {
            int curr = data[i];
            ...
            if (type == 0) {
                continue;
            } else if (type > 1 && i + type <= len) {  // for type 2,3,4,5
            	// type-- is the same thing as the solution1, to let i++ in order to check all the
            	// numbers of this type. Type 1 is 10000000
                while (type-- > 1) {
                    if (getType(data[++i]) != 1) {
                        return false;
                    }
    ...
    // masks is: 128, 64, 32, 16, 8
    // valid byte type: 0**, 10**, 110**,1110** and 11110**
    // 11100000
    // 11000000  
    // 10000000  (128)
    // for example: 10000010 (130)
	...
	public static void main(String[] args) {
		int[]a = {197, 130, 1};         
		System.out.println(validUtf8I(a));   //yes 
		
		int[]b = {235, 140, 4};
		System.out.println(validUtf8I(b));  //no
		
		int[]c = {4};
		System.out.println(validUtf8I(c));   //yes
	}
}